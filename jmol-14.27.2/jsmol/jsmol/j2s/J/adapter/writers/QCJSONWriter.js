...
Clazz.load (["JU.SB", "J.io.JSONWriter", "java.util.Hashtable"], "J.adapter.writers.QCJSONWriter", ["java.lang.Boolean", "java.util.Date", "JU.DF", "$.P3", "$.PT", "J.adapter.writers.QCSchema", "J.quantum.SlaterData"], function () {
...
}, J.adapter.writers, "QCJSONWriter", J.io.JSONWriter);
Clazz.prepareFields (c$, function () {
this.moBases =  new java.util.Hashtable ();
});
c$.getUnitsConversion = Clazz.defineMethod (c$, "getUnitsConversion", 
function (units, asArray) {
var sunits = J.adapter.writers.QCSchema.getFactorToAU (units);
return (asArray ?  Clazz.newArray (-1, [units, sunits]) : "[\"" + units + "\"," + sunits + "]");
}, "~S,~B");
...
function () {
this.writeString (J.adapter.writers.QCSchema.version);
});
...
this.mapAddKeyValueRaw (prefix + "_units", J.adapter.writers.QCJSONWriter.getUnitsConversion (units, false), ",\n");
}, "~S,~S");
...
this.mapAddKeyValueRaw (key, "{\"value\":" + value + ",\"units\":" + J.adapter.writers.QCJSONWriter.getUnitsConversion (units, false) + "}", ",\n");
}, "~S,~O,~S");
...
if (units == null) units = "?";
moDataJSON.put ("orbitals_energy_units", J.adapter.writers.QCJSONWriter.getUnitsConversion (units, true));
